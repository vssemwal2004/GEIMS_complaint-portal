...
const EmployeeDashboard = () => {
    ...
    const fetchData = async () => {
      try {
        const [statsRes, complaintsRes] = await Promise.all([
          ...
          api.get('/api/employee/complaints?limit=5'),
        ]);
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="bg-teal-50/40 border border-teal-100 rounded-xl px-4 py-4">
        <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="min-w-0">
          <h1 className="text-xl font-semibold leading-tight text-gray-900 flex items-baseline gap-2 flex-wrap">
            <span className="inline-flex items-baseline gap-2">
              <span className="welcome-word welcome-word-0">Welcome</span>
              <span className="welcome-word welcome-word-1">back</span>
            </span>
            <span className="text-teal-700 truncate">{user?.name}</span>
          </h1>
          <p className="text-sm text-gray-700 mt-1">Employee complaint dashboard summary</p>
        </div>
        <Link
          href="/employee/submit-complaint"
          className="inline-flex items-center gap-2 px-4 py-2 bg-teal-600 text-white text-sm font-medium rounded-lg hover:opacity-90 whitespace-nowrap"
        >
          <FiPlusCircle size={16} />
          Submit
        </Link>
        </div>
        <div className="h-px bg-teal-100 mt-4" />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-teal-50/30 border border-gray-200 rounded-xl">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                ...
                <span className="inline-flex items-center gap-2 px-2.5 py-1 rounded-full text-xs font-medium bg-yellow-50 text-yellow-700">
                  ...
                  Review: {activity.recentStatusCounts.UNDER_REVIEW}
                </span>
      ...
      <div className="bg-white border border-gray-200 rounded-xl">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
          ...
          <Link href="/employee/complaints" className="text-sm text-gray-600 hover:text-teal-800 font-medium">
            View all
          </Link>
        </div>
